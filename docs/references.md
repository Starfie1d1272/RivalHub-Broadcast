...
RivalHub Broadcast 不 fork 单一 HUD Manager。现有项目作为产品、架构、协议、性能和运维参考；真正进入代码库前仍需单独确认许可证、维护状态和当前 commit。

| 项目 | 主要学习点 | 不直接继承的部分 |
| --- | --- | --- |
| `drweissbrot/cs-hud` | 最小 GSI → local server → HUD/config/radar → Browser Source；local-first | 老旧技术结构与有限 scene/product model |
| `mortenlein/eon` | Waiting/Intermission/Result、Operator、Simulator、Radar、provisional analytics、Playwright；长期运行/重连经验 | 不 fork 其逐步演化出的 monolithic GSI/server 结构；具体缺口判断必须按当前代码重新核实，不能只引用旧 audit |
| `nsnsay/Zhenhai-HUD-Manager` | TypeScript workspace、`csgogsi`、grenade/smoke/inferno、上下层 Radar、listener lifecycle | App 本体 repo-level license 边界不清；不复制应用代码/资产，也不继承第二套 Team/Match DB |
| 旧 Void / Void-HUD-Overlay | 高频 pipeline 的性能反例 | `structuredClone` 全 payload + recursive traversal + producer>consumer backlog |
| Boltobserv | 专业 observer Radar、advanced player states、autozoom、observer advisory、上下层、z-height、utility、Browser/OBS 输出 | GPL-3.0；不作为 sidecar/第二套 GSI runtime，不复制其应用实现；重点吸收 Radar 产品/算法经验 |
| Lexogrine Obserview | 独立 Radar UX、avatar marker、telestrator、跨电脑 drawing sync | **license metadata inconsistent**：根 `LICENSE` 为 MIT，但 `package.json` 标注 GPL-3.0；代码级复用前必须确认；依赖 LHM/csgogsi-socket 且技术栈较旧 |
| Lexogrine HUD Manager | 成熟 production UX、HUD package、panel/action、veto/camera/replay 生态 | Manager 本体为自有 EULA，不作为代码来源 |
| `lexogrine/cs2-react-hud` | 开源 HUD package / panel / action contract、custom radar/killfeed | 不让 LHM contract 成为内部 RuntimeState / Broadcast protocol |
...
Radar 不等于地图 calibration，也不等于某一个现成 Radar 应用：
...
6. 已存在明确 owner 的共享基础设施应优先通过 adapter/provider 复用，而不是在 Broadcast 再维护一份真相。
7. 对“当前实现是否仍存在某缺口”的判断，如果依赖具体代码行为，应记录 verified date；长期重要或容易变化的结论可额外记录 commit SHA，不要求每条 UX 观察都 pin commit。
