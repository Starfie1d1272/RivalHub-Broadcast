...
- freshness 由当前 generation 的 accepted snapshot 与 monotonic receive time 派生；没有当前 baseline（包括 generation advance 或 map reset 之后）为 `awaiting`，超过调用方注入的 `staleAfterMs` 才为 `stale`；selector clock 早于 receive cursor 时直接 fail-fast。Core 不保存 timer/stateful `isStale`；Companion ProjectionCoordinator 只通过一个可注入的 stale-deadline scheduler 触发重新发布，让已接受的 frame 在 deadline 后及时暴露为 stale。
