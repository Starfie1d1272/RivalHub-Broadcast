...
6. 精确 dependency version 由 `package.json` / lockfile 记录；同一既定技术路线内的兼容 patch/minor 升级通常不需要新 ADR，除非改变架构语义或生产兼容边界。
...
- [`0003-runtime-state-delivery-invariants.md`](0003-runtime-state-delivery-invariants.md)：RuntimeState/projection、session/identity、delivery/backpressure、observation outbox、Radar/scene 与跨仓 contract invariant；其中跨仓 contract ownership 部分 supersede ADR-0002 决策 6 的过宽表述。