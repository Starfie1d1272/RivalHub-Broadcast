...
`/` 重定向到 `/operator`。制作控制、HUD 编辑器与运行诊断共享导航；播出画面是单独打开的透明输出，不进入产品导航 tab。未知用户路径显示中文 404。制作首页复用 OperatorSnapshot 展示比赛、系列赛、数据与异常；地图绑定仅在需要制作确认时显示。诊断首页提供中文状态，原始证据收进高级技术信息。现场验收继续由 Companion 独立提供，普通运行时只提供进入说明，不自动启动验收。OBS 浏览器源状态来自本地 WebSocket 的浏览器标识，只表示观察到相应连接，不能证明画面已加载或可见。
