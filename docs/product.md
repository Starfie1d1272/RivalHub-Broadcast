...
RivalHub 是第一方、信息最完整的赛事上下文提供方，但不是运行 HUD、雷达或观察辅助的前置条件。
...
7. **开源与可移植。** Core、Radar、协议和 Lookahead 语义不绑定某个云端后台或某个第三方 HUD Manager。

## 3. 产品体验核心：制播工作区

传统 HUD 工作流通常把游戏、网页控制、雷达、诊断和辅助信息分散在多个窗口。RivalHub Broadcast 的长期产品形态是一套**私有制播工作区**：
...
制播工作区是产品体验范式，不等于固定布局，也不要求某一种桌面框架。浏览器、OBS Browser Source、透明窗口或桌面容器只是承载方式。
...
Program 投影负责完成领域解释；React 组件只负责展示。例如选手存活状态由 Core 统一推导为 ``alive | dead | unknown``，渲染层不再次根据 HP 猜测。
...
观察辅助利用比正式节目更早的时间轴，把已经真实发生、但延迟节目尚未到达的事件转换成低认知负担的提示。
...
这些形态都消费 ``ObserverAssistProjection``。它们不能与正式 Program 混在同一个安全边界内，更不能依赖 OBS crop 或窗口层级来“隐藏”未来信息。
...
高影响 canonical 操作不能在离线时静默排队后自动执行。可靠 observation 可以进入有界持久投递队列，但发送前必须重新验证会话、地图执行和身份上下文。
...
术语规范见 [`terminology.md`](terminology.md)。
...
产品扩展应优先通过明确的适配器、能力接口或独立消费投影接入，而不是扩张一个万能状态对象。
