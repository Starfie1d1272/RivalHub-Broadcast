...
const defaultScheduler: ReplaySessionScheduler = {
...
export class ReplaySession<Frame extends ReplaySessionFrame> {
  ...
  constructor(source: ReplaySessionSource<Frame>, scheduler = defaultScheduler) {
  ...
  play(speed = 1): void {
    ...
    if (this.snapshot.currentIndex >= this.source.frames.length - 1) return;
    ...
    this.playbackAnchorMs =
      this.scheduler.nowMs() - (currentElapsedUs - firstElapsedUs) / 1000 / speed;
  ...
  private scheduleNextFrame(): void {
    ...
    const currentFrame = this.snapshot.current;
    if (nextFrame === undefined || currentFrame === null) {
      this.update({ isPlaying: false });
      return;
    }
    const deadline =
      ...
      (nextFrame.cursor.scheduledElapsedUs - this.source.frames[0]!.cursor.scheduledElapsedUs) /
        1000 /
        this.speed;
    const delayMs = Math.max(0, deadline - this.scheduler.nowMs());
