...
export function checksFrom({ markers, finalRuntime, captureResults, captureErrors, artifact }) {
  ...
  const checks = {
    productionChain: {
      label: '第一场数据进入 production chain',
      status: productionChainPassed ? 'PASS' : 'INCONCLUSIVE',
      reason: productionChainPassed
        ? 'Demo A marker 已绑定 reset 前同一 map execution 的 accepted Capture V1 frame。'
        : '缺少与 Demo A marker 同一 map execution、同一 sequence/timestamp 的 Capture V1 frame。',
    },
    realSilenceToStale: {
      label: '停止输入后进入 stale',
      status: stopPassed ? 'PASS' : 'INCONCLUSIVE',
      reason: stopPassed
        ? 'Companion 在未重启的情况下观察到 runtime-stale，并在下一场 reset 前记录了 cs2-closed。'
        : '等待 Demo A 后 runtime 进入 stale，并在下一场 reset 前确认 cs2-closed。',
    },
    explicitNextExecution: {
      label: '下一场从显式新 map execution 开始',
      status: resetPassed ? 'PASS' : 'INCONCLUSIVE',
      reason: resetPassed
        ? 'mapEpoch 已推进、producerInstanceId 保持一致，且上一场 Program telemetry 已清理。'
        : '缺少成功的 explicit next-execution reset 证据。',
    },
    demoBRecovery: {
      label: '第二场恢复且无上一场残留',
      status: demoBRecoveryPassed ? 'PASS' : 'INCONCLUSIVE',
      reason: demoBRecoveryPassed
        ? 'Demo B marker 已绑定 reset 后新 map execution 的 accepted frame，并恢复 fresh。'
        : '等待 CS2 重开、reset 后新 map execution 的 Demo B frame 与 fresh final runtime。',
    },
    captureIntegrity: {
      label: 'Capture V1 完整性',
      status: captureFailed ? 'FAIL' : captureResults.length > 0 ? 'PASS' : 'INCONCLUSIVE',
      reason: captureFailed
        ? 'Capture V1 不完整、发生 dropped frame 或完整性校验失败。'
        : captureResults.length > 0
          ? 'Capture V1 frame count、hash 与 schema 校验通过。'
          : '没有找到已发布的 Capture V1。',
    },
  };
