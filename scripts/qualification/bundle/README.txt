...
3. 执行：`powershell -ExecutionPolicy Bypass -File .\scripts\start.ps1`；这会启动常规现场验收。若要进行目标时钟专项验收，执行：`powershell -ExecutionPolicy Bypass -File .\scripts\start.ps1 -ObjectiveTiming`。
...
目标时钟专项验收的八类场景需要使用 `scripts\mark.ps1` 记录开始与结束。重连或接收端重启场景的操作顺序是：先在已下包或拆弹状态记录 `-Phase before`，实际重连或重启接收端后执行 `scripts\rotate.ps1`，等待新的已下包观测，再记录 `-Phase after`。`rotate.ps1` 会在同一轮现场验收中开始新的采集记录并推进 Program 数据来源代际，不会新建验收轮次；没有新代际的正常观测时，结束标记会被拒绝。
